    ...
    <Layout title="Pustaka Media (R2 Vault)" currentPath={currentPath}>
      <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
        ...
        <div class="md:col-span-1">
          <form id="upload-form" action="/api/upload" method="POST" enctype="multipart/form-data" class="bg-dark-800/50 backdrop-blur-sm border border-slate-700/50 rounded-2xl p-6 text-center shadow-xl border-dashed h-full flex flex-col justify-center">
            
            <div id="preview-container" class="hidden mb-6 relative rounded-xl overflow-hidden border border-slate-700/50 bg-dark-900 aspect-square">
              ...
              <div class="absolute inset-0 bg-black/40 flex items-center justify-center opacity-0 hover:opacity-100 transition">
                <button type="button" onclick="resetUpload()" class="bg-red-500/80 text-white px-3 py-1.5 rounded-lg text-sm font-medium backdrop-blur-sm">Hapus</button>
              </div>
            </div>

            <div id="upload-prompt" class="flex flex-col items-center">
              <div class="w-20 h-20 bg-dark-900 rounded-full flex items-center justify-center mb-4 border border-slate-700/50 shadow-inner">
                <svg class="w-8 h-8 text-brand-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-8l-4-4m0 0L8 8m4-4v12"></path></svg>
              </div>
              <h2 class="text-lg font-semibold text-white mb-2">Unggah Aset Media</h2>
            ...
            <div id="watermark-options" class="hidden mt-6 text-left border-t border-slate-700/50 pt-4">
              <label class="flex items-center gap-2 cursor-pointer mb-3">
                <input type="checkbox" id="apply-watermark" checked class="rounded border-slate-600 bg-slate-800 text-brand-500 focus:ring-brand-500/20" onchange="updatePreview()" />
                <span class="text-slate-300 text-sm font-medium">Beri Watermark Hak Cipta</span>
              </label>
              <input type="text" id="watermark-text" value="© Nuansa Studio" oninput="updatePreview()" class="w-full bg-dark-900 border border-slate-700 rounded-lg px-3 py-2 text-sm text-white focus:border-brand-500 focus:outline-none placeholder-slate-600" />
              <p class="text-[10px] text-slate-500 mt-2 leading-tight">Watermark dirender secara lokal menggunakan HTML5 Canvas sehingga 100% gratis tanpa biaya Serverless.</p>
        ...
        <div class="md:col-span-2">
          <div class="bg-dark-800/50 backdrop-blur-sm border border-slate-700/50 rounded-2xl p-6 shadow-xl h-full">
            ...
            {files.length === 0 ? (
              <div class="text-center p-12 border border-slate-700/50 border-dashed rounded-xl text-slate-500 text-sm">
