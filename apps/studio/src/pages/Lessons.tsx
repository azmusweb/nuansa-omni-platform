...
export const Lessons: FC<{ currentPath: string; course: Course; lessons: Lesson[] }> = ({ currentPath, course, lessons }) => {
 ...
 <a href="/learn" class="text-gray-400 hover:text-brand-600 transition text-sm flex items-center gap-1">
 <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18"></path></svg>
 Semua Kursus
 </a>
 <span class="text-gray-700 ">/</span>
 <span class="text-slate-700 text-sm font-medium">{course.title}</span>
 </div>
 ...
 <div class="space-y-3">
 {sorted.length === 0 ? (
 <div class="bg-white shadow-sm rounded-2xl border border-slate-200 p-12 text-center">
 <p class="text-gray-400 ">Belum ada materi. Tambahkan materi pertama untuk kursus ini.</p>
 </div>
 ) : sorted.map((lesson, idx) => (
 <div class="bg-white shadow-sm rounded-2xl border border-slate-200 p-4 flex items-center gap-4">
 ...
 <span class="font-semibold text-slate-900 truncate">{lesson.title}</span>
 {lesson.is_preview ? (
 <span class="text-xs px-2 py-0.5 rounded-full bg-emerald-100 text-emerald-700 shrink-0">Preview Gratis</span>
 ) : (
 <span class="text-xs px-2 py-0.5 rounded-full bg-amber-100 text-amber-700 shrink-0">Premium</span>
 )}
 </div>
 </div>
 <div class="flex gap-2 shrink-0">
 <button onclick={`openLessonModal('${lesson.id}','${lesson.title.replace(/'/g,"\\'")}','${encodeURIComponent(lesson.content||'')}',${lesson.order_index},${lesson.is_preview})`} class="px-3 py-1.5 text-xs rounded-lg bg-slate-100 text-slate-600 hover:bg-slate-200 transition">Edit</button>
 <form action="/api/learn/lesson/delete" method="POST" class="inline" onsubmit="return confirm('Hapus materi ini?')">
 <input type="hidden" name="id" value={lesson.id} />
 <input type="hidden" name="course_id" value={lesson.course_id} />
 <button type="submit" class="px-3 py-1.5 text-xs rounded-lg bg-red-50 text-red-500 hover:bg-red-100 transition">Hapus</button>
 </form>
 </div>
 </div>
 ))}
 ...
 <div class="bg-white border border-slate-200 rounded-2xl w-full max-w-2xl max-h-[90vh] flex flex-col shadow-2xl">
 <div class="flex justify-between items-center p-5 border-b border-slate-200 shrink-0">
 <h3 id="modal-lesson-title" class="font-bold text-lg text-slate-900 ">Tambah Materi</h3>
 <button onclick="closeLessonModal()" class="text-gray-500 hover:text-slate-700 ">
 ...
 <div>
 <label class="block text-xs font-semibold text-gray-400 uppercase tracking-wider mb-1">Judul Materi</label>
 <input type="text" name="title" id="lesson-title" required placeholder="Misal: Pengenalan SEO" class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm text-slate-900 focus:outline-none focus:border-brand-500 transition" />
 </div>
 <div>
 <label class="block text-xs font-semibold text-gray-400 uppercase tracking-wider mb-1">Urutan</label>
 <input type="number" name="order_index" id="lesson-order" placeholder="1" class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm text-slate-900 focus:outline-none focus:border-brand-500 transition" />
 </div>
 </div>
 <div>
 ...
 <div class="w-10 h-5 bg-slate-300 rounded-full peer peer-checked:after:translate-x-full after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:rounded-full after:h-4 after:w-4 after:transition-all peer-checked:bg-indigo-500"></div>
 </div>
 <span class="text-sm text-slate-700 ">Preview Gratis (bisa dibaca tanpa daftar)</span>
 </label>
 </div>
 <div>
