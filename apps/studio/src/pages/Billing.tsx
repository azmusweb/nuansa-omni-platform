    ...
    <Layout title="Layanan Tambahan (Billing)" currentPath={currentPath}>
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="col-span-1 lg:col-span-2 space-y-6">
          <div class="bg-dark-800 border border-slate-700/50 rounded-2xl shadow-xl p-8">
            <h2 class="text-2xl font-bold text-white mb-2">Paket Langganan Anda</h2>
            <p class="text-slate-400 mb-6">Kelola paket, perbarui lisensi, dan beli fitur tambahan.</p>
            
            <div class="flex items-center gap-4 p-4 bg-dark-900 border border-slate-700 rounded-xl mb-8">
              <div class="w-12 h-12 rounded-full bg-brand-500/20 text-brand-400 flex items-center justify-center font-bold uppercase">
                {plan[0]}
              </div>
              <div>
                <p class="text-sm text-slate-400">Paket Saat Ini</p>
                <p class="text-xl font-bold text-white uppercase tracking-wider">{plan}</p>
              </div>
            </div>

            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
              {/* Kartu Upgrade Pro */}
              <div class="border border-slate-700 rounded-xl p-5 hover:border-brand-500 transition relative overflow-hidden group">
                ...
                <h3 class="text-lg font-bold text-white mb-1">Paket Pro</h3>
                <p class="text-xl font-bold text-brand-400 mb-4">Rp 199.000<span class="text-sm font-normal text-slate-500">/bln</span></p>
                ...
                <form action="/api/buy" method="POST">
                  ...
                  <button type="submit" class="w-full py-2 bg-dark-900 border border-slate-600 hover:border-brand-500 text-white rounded-lg font-medium text-sm transition" disabled={plan === 'pro' || plan === 'enterprise'}>
              ...
              <div class="border border-brand-500/50 bg-brand-500/5 rounded-xl p-5 hover:border-brand-500 transition relative overflow-hidden group">
                <div class="absolute top-0 right-0 bg-brand-500 text-white text-[10px] font-bold px-2 py-1 rounded-bl-lg">TERPOPULER</div>
                <h3 class="text-lg font-bold text-white mb-1">Enterprise</h3>
                <p class="text-xl font-bold text-brand-400 mb-4">Rp 499.000<span class="text-sm font-normal text-slate-500">/bln</span></p>
                <ul class="text-sm text-slate-400 space-y-2 mb-6">
                ...
                <form action="/api/buy" method="POST">
                  ...
                  <button type="submit" class="w-full py-2 bg-brand-600 hover:bg-brand-500 text-white rounded-lg font-medium text-sm transition shadow-lg shadow-brand-500/20" disabled={plan === 'enterprise'}>
        ...
        <div class="col-span-1 space-y-6">
          <div class="bg-dark-800 border border-slate-700/50 rounded-2xl shadow-xl overflow-hidden">
            <div class="p-5 border-b border-slate-700/50 bg-dark-900/50">
              <h3 class="font-semibold text-white">Status Permintaan</h3>
            </div>
            <div class="p-0">
              <ul class="divide-y divide-slate-700/50">
                {transactions.length === 0 ? (
                  <li class="p-5 text-center text-sm text-slate-500">Belum ada transaksi atau permintaan pending.</li>
                ) : (
                  transactions.map((tx) => (
                    <li class="p-5 flex flex-col gap-2 hover:bg-dark-700/20 transition">
                      <div class="flex justify-between items-start">
                        <span class="text-sm font-medium text-white">{tx.type === 'plan_upgrade' ? 'Upgrade Paket' : tx.type}</span>
                      ...
                      <span class="text-xs text-slate-400 font-mono">ID: {tx.id.split('-')[0]}</span>
                      {tx.details && (
                        <span class="text-xs text-slate-500">{JSON.parse(tx.details).target_plan?.toUpperCase()}</span>
                      )}
                    </li>
                  ))
                )}
