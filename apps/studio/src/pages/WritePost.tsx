...
export const WritePost: FC<{ currentPath: string, post?: any }> = ({ currentPath, post }) => {
 ...
 <div id="top-bar" class="sticky top-0 z-50 bg-white border-b border-slate-200 px-4 py-2 flex items-center justify-between gap-4 shadow-sm">
 
 {/* Left: Back & Title */}
 <div class="flex items-center gap-3 flex-1 min-w-0">
 <a href="/posts" class="flex items-center justify-center w-10 h-10 rounded-full hover:bg-slate-100 text-gray-400 transition shrink-0" aria-label="Kembali">
 ...
 maxlength={200}
 placeholder="Judul"
 class="w-full bg-transparent border-none outline-none text-slate-800 placeholder-slate-400 text-lg sm:text-xl font-medium px-2 py-1 focus:ring-0"
 />
 </div>
 ...
 {/* Mobile tab toggle */}
 <div class="flex lg:hidden bg-slate-100 rounded-lg p-0.5 border border-slate-200 ">
 ...
 <button type="button" class="hidden sm:flex items-center gap-2 text-slate-600 bg-white border border-slate-300 hover:bg-slate-50 px-4 py-1.5 rounded text-sm font-medium transition">
 ...
 {/* ===== MAIN FORM ===== */}
 <form id="post-form" action="/api/posts" method="POST" onsubmit="event.preventDefault(); submitPost();" class="flex-1 flex flex-col h-full bg-slate-50 ">
 <div class="flex flex-col lg:flex-row flex-1 h-full w-full max-w-screen-2xl mx-auto">

 {/* ===== EDITOR PANEL ===== */}
 <div id="panel-editor" class="flex-1 flex flex-col min-w-0 h-[calc(100vh-60px)] lg:h-auto border-r border-slate-200 ">
 <div id="editor-area" class="flex-1 bg-white ">
 <textarea id="editorjs" class="w-full"></textarea>
 </div>
 {/* Word count bar */}
 <div class="border-t border-slate-200 bg-white px-4 py-1.5 flex items-center justify-between text-xs text-gray-400">
 ...
 <div id="panel-settings" class="w-full lg:w-[320px] shrink-0 bg-white overflow-y-auto hidden lg:flex flex-col h-[calc(100vh-60px)] lg:h-auto">
 
 <div class="p-4 border-b border-slate-200 ">
 <h2 class="text-sm font-semibold text-slate-800 uppercase tracking-wide">Setelan Postingan</h2>
 </div>
 ...
 <span class="text-sm font-medium text-slate-700 ">Status Publikasi</span>
 <svg id="status-chevron" class="w-4 h-4 text-gray-500 group-hover:text-slate-600 transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"/></svg>
 </button>
 <div id="status-body" class="px-4 pb-4">
 <select name="status" id="post-status" class="w-full bg-transparent border-b border-slate-300 rounded-none px-0 py-2 text-slate-800 text-sm focus:outline-none focus:border-brand-500 transition cursor-pointer">
 ...
 <span class="text-sm font-medium text-slate-700 ">Gambar Utama</span>
 <svg id="image-chevron" class="w-4 h-4 text-gray-500 transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"/></svg>
 </button>
 ...
 <span class="text-sm font-medium text-slate-700 ">Deskripsi Penelusuran</span>
 <svg id="seo-chevron" class="w-4 h-4 text-gray-500 transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"/></svg>
 </button>
 <div id="seo-body" class="px-4 pb-4 section-collapsed">
 <div class="mb-3">
 <label class="block text-gray-400 text-xs mb-1">Judul Khusus (opsional)</label>
 <input type="text" id="meta-title" placeholder="Judul SEO..." class="w-full bg-transparent border-b border-slate-300 px-0 py-1.5 text-sm text-slate-800 focus:border-brand-500 focus:outline-none transition" />
 </div>
 <div>
 <label class="block text-gray-400 text-xs mb-1">Deskripsi Penelusuran</label>
 <textarea id="meta-desc" rows={3} placeholder="Deskripsi singkat..." class="w-full bg-transparent border border-slate-300 rounded px-2 py-1.5 text-sm text-slate-800 focus:border-brand-500 focus:outline-none transition resize-none"></textarea>
 <div class="text-right text-xs text-gray-500 mt-1"><span id="meta-desc-count">0</span>/160</div>
 </div>
 </div>
 </div>
 ...
 <span class="text-sm font-medium text-slate-700 ">Opsi Monetisasi</span>
 <svg id="mono-chevron" class="w-4 h-4 text-gray-500 transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"/></svg>
 </button>
 ...
 <div>
 <div class="text-slate-700 text-sm font-medium">Jadikan Premium</div>
 <div class="text-gray-400 text-xs mt-0.5">Berbayar untuk dibaca penuh</div>
 </div>
 </label>
 <div id="price-container" class={`mt-3 transition-all ${isPremium ? '' : 'hidden'}`}>
 <label class="block text-gray-400 text-xs mb-1">Harga (Rp)</label>
 <input type="number" id="post-price" name="price" value={price || ''} min="0" placeholder="5000" class="w-full bg-transparent border-b border-slate-300 px-0 py-1.5 text-sm text-slate-800 focus:border-brand-500 focus:outline-none transition" />
 </div>
 </div>
 </div>
