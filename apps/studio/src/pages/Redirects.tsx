...
export const Redirects: FC<{ currentPath: string, redirects: any[] }> = ({ currentPath, redirects = [] }) => {
 ...
 <td class="px-6 py-4 text-sm text-gray-900 font-mono break-all">{r.source_url}</td>
 <td class="px-6 py-4 text-sm text-gray-700 font-mono break-all">{r.target_url}</td>
 <td class="px-6 py-4">
 <span class={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
 r.status_code === 301 ? 'bg-brand-500/10 text-brand-600 border border-gray-200' : 'bg-orange-500/10 text-orange-400 border border-orange-500/20'
 }`}>
 ...
 <form action="/api/redirects" method="POST" class="bg-white border border-gray-200 rounded-2xl shadow-xl overflow-hidden">
 ...
 <input type="text" name="source_url" required class="w-full bg-gray-50 border border-gray-200 rounded-xl pl-6 pr-4 py-2.5 text-gray-900 text-sm font-mono focus:border-brand-500 focus:ring-1 focus:ring-brand-500 transition" placeholder="artikel-lama" />
 </div>
 ...
 <input type="text" name="target_url" required class="w-full bg-gray-50 border border-gray-200 rounded-xl pl-6 pr-4 py-2.5 text-gray-900 text-sm font-mono focus:border-brand-500 focus:ring-1 focus:ring-brand-500 transition" placeholder="artikel-baru" />
 </div>
 ...
 <select name="status_code" class="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-2.5 text-gray-900 text-sm focus:border-brand-500 focus:ring-1 focus:ring-brand-500 transition">
