...
export const Layout: FC<{ title: string; currentPath: string }> = ({ title, currentPath, children }) => {
 ...
 return (
 <a href={sub.href} class={`block py-2 px-3 text-sm rounded-lg transition-colors ${isSubActive ? 'text-brand-600 bg-brand-500/10 font-medium' : 'text-gray-500 hover:text-brand-700 hover:bg-brand-50'}`}>
 {sub.label}
 </a>
 )
 })}
 ...
 <button class="md:hidden text-gray-500 p-2" onclick="document.getElementById('studio-sidebar').classList.add('-translate-x-full'); document.getElementById('studio-backdrop').classList.add('hidden');">
 ...
 <h1 class="text-xl md:text-2xl font-bold text-brand-600 tracking-tight truncate max-w-[150px] md:max-w-none">&gt; {title.toUpperCase()}</h1>
 </div>
 ...
 submit.type = 'submit';
 submit.className = \`w-full text-left px-3 py-2 rounded-lg text-sm transition-colors flex items-center justify-between \${isActive ? 'bg-brand-500/20 text-brand-600 font-medium' : 'text-gray-700 hover:bg-gray-100'}\`;
 submit.innerHTML = \`
 <span>\${tenant.name}</span>
 \${isActive ? '<svg class="w-4 h-4 text-brand-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path></svg>' : ''}
 \`;
